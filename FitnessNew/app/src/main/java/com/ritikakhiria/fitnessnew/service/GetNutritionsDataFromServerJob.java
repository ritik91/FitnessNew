...
public class GetNutritionsDataFromServerJob extends Job {
    String mid;
    public static final String TAG = GetNutritionsDataFromServerJob.class.getCanonicalName();

    public GetNutritionsDataFromServerJob(String id) {
    ...
    public void onRun() throws Throwable {
        ...
        params.put("id", mid);
        Call<MyResponseNutrition> request = service.getNutrition(params);
        MyResponseNutrition product = AppRetrofitManager.performRequest(request);
        if (product != null && product.getResponse() == 200) {
            DBHelper.getInstance(getApplicationContext()).insertNutritionNewFromServer(product.getResult1());
        } else {
